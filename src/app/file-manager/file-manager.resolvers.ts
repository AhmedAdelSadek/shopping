...
export class FileManagerItemsResolver implements Resolve<any>
{
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Item[]> {
        return this._fileManagerService.getItems().pipe(map(values => {
            console.log(values);
            return values;
        }))
    }
...
export class FileManagerFolderResolver implements Resolve<any>
{
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Item[]> {
        ...
        return this._fileManagerService.getItems(folderId)
            .pipe(map(values => {
                console.log(values);
                return values;
            }),
...
export class FileManagerItemResolver implements Resolve<any>
{
    ...
    resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<Item> {
        ...
        console.log(id)
        return this._fileManagerService.getItemById(id)
            .pipe(
                // Error here means the requested task is not available
                catchError((error) => {
